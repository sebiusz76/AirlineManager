...
namespace AirlineManager.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> EnableSecondFactorAjax([FromBody] JsonElement payload)
        {
            ...
            // Log audit: 2FA Enabled by self
            await LogUserChange(user.Id, user.Email, "2FAEnabled",
                oldValues: new { TwoFactorEnabled = false },
                newValues: new { TwoFactorEnabled = true, RecoveryCodesGenerated = 10 },
                changes: "Two-factor authentication enabled by user");

            _logger.LogInformation("Two-factor authentication enabled for user {Email}", user.Email);
        ...
        public async Task<IActionResult> DisableTwoFactorSelf()
        {
            ...
            // Log audit: 2FA Disabled by self
            await LogUserChange(user.Id, user.Email, "2FADisabled",
                oldValues: new { TwoFactorEnabled = true },
                newValues: new { TwoFactorEnabled = false },
                changes: "Two-factor authentication disabled by user");

            _logger.LogInformation("Two-factor authentication disabled for user {Email}", user.Email);
        ...
        public async Task<IActionResult> ResetRecoveryCodesAjax()
        {
            ...
            // Log audit: Recovery Codes Reset
            await LogUserChange(user.Id, user.Email, "RecoveryCodesReset",
                newValues: new { RecoveryCodesGenerated = 10 },
                changes: "Two-factor recovery codes regenerated by user");
